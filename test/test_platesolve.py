...
from src.platesolve import ASTAPSolver, create_hint, SolverError, SolverFailure
import pytest
...
def test_solve_asi183mm():
    ...
    assert headers["CRVAL1"] == 185.7244712821
    assert headers["CRVAL2"] == 15.81205749322


def test_solve_asi294mc():
    headers = solve_testFile("L_2018-10-23_20-58-50_Bin1x1_240s__-20C.fit")
    assert headers["PLTSOLVD"] == True
    assert headers["CRVAL1"] == 341.8189226876
    assert headers["CRVAL2"] == 58.12444446502


def test_solve_asi294mc_135mm():
    headers = solve_testFile("Sadr Area_2020-05-28T011655_60sec_HaOIII__-15C_frame18.fit")
    assert headers["PLTSOLVD"] == True
    assert headers["CRVAL1"] == pytest.approx(305.185298889526, 0.01)
    assert headers["CRVAL2"] == pytest.approx(40.284375, 0.01)
...
def test_solve_asi294mc_blind():
    hint = create_hint(324, 57)  # this is only to speed up the test. it should still pass if hint = None
    # hint = None
    ...
    assert headers["CRVAL1"] == pytest.approx(324.676522676, 0.01)
    assert headers["CRVAL2"] == pytest.approx(57.5042416425, 0.01)


def test_solve_asi290mm_blind():
    hint = create_hint(250, 85)  # this is only to speed up the test. it should still pass if hint = None
    ...
    assert headers["CRVAL1"] == pytest.approx(251.2511043023, 0.01)
    assert headers["CRVAL2"] == pytest.approx(89.04982022846, 0.01)
...
def test_nosolution():
    ...
    assert "No solution found!" in excinfo.value.log
